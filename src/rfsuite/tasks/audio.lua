...
function AudioTask:_adjustmentFiles(adjFunc)
    ...
    if type(wavs) ~= "string" then
        return nil
    end

    self._adjustmentFileCache = self._adjustmentFileCache or {}
    files = self._adjustmentFileCache[adjFunc]

    if files == nil then
        files = {}
        i = 0
        for token in wavs:gmatch("[^%s]+") do
            i = i + 1
            files[i] = token .. ".wav"
        end
        self._adjustmentFileCache[adjFunc] = files
    end
