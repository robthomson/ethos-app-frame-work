...
local function getMspQueue()
    return context.msp and context.msp.mspQueue or nil
end
...
local function resolveWriteUUID(spec, state)
    ...
    if spec.writeUuidFallback == true or spec.writeUuidFallback == "unique" then
        return context.rfsuite.utils.uuid() or tostring(os_clock())
    end
...
function factory.create(spec)
    ...
    local function read(...)
        ...
        message = {
            ...
            structure = spec.readStructure
        }

        if type(spec.resolveReadUUID) == "function" then
            message.uuid = spec.resolveReadUUID(state, ...)
        end
        if payload ~= nil then
            message.payload = payload
        end
        if type(spec.resolveReadTimeout) == "function" then
    ...
    local function write(suppliedPayload, ...)
        ...
        if spec.writeRequiresStructure == true and suppliedPayload == nil and type(spec.writeStructure) == "table" and #spec.writeStructure == 0 then
            return false, "write_not_implemented"
        end

        if type(spec.validateWrite) == "function" then
            local ok, reason = spec.validateWrite(suppliedPayload, state, ...)
            if ok == false then
        ...
        if type(spec.resolveWriteUUID) == "function" then
    ...
    if type(spec.methods) == "table" then
        for name, fn in pairs(spec.methods) do
            if type(fn) == "function" then
                api[name] = function(...)
                    return fn(state, ...)
                end
    ...
    if type(spec.exports) == "table" then
        for name, value in pairs(spec.exports) do
            api[name] = value
        end
    end
