...
local function createApiReadHook(spec)
    return {
        ...
        wakeup = function(context)
            local state = context.hook._apiState or {started = false, failed = false}
            local api
            local loadErr
            local ok
            local err
        ...
        isComplete = function(context)
            local state = context.hook._apiState
...
local HOOKS = {
    flightmode = {
        wakeup = flightmodeHook,
        isComplete = function(_, result)
            return result ~= false
        end
    },
    fcversion = createApiReadHook({
        name = "fcversion",
        apiName = "FC_VERSION",
    ...
    telemetryconfig = createApiReadHook({
