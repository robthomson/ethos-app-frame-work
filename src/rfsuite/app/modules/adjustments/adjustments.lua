...
local function loadHelpLines()
    local ok, data = pcall(loadChunk, "app/modules/adjustments/help.lua")
...
function Page:open(ctx)
    local legacy = loadChunk("app/modules/adjustments/legacy.lua")
    local helpLines = loadHelpLines()
    local node = {
        baseTitle = ctx.item.title or legacy.title or "@i18n(app.modules.adjustments.name)@",
        title = ctx.item.title or legacy.title or "@i18n(app.modules.adjustments.name)@",
        subtitle = ctx.item.subtitle or "Adjustment ranges",
    ...
    function node:help()
        ...
        if type(helpLines) == "table" then
            return diagnostics.openHelpDialog((self.title or self.baseTitle or "@i18n(app.modules.adjustments.name)@") .. " Help", helpLines)
        end
        return false
    end
