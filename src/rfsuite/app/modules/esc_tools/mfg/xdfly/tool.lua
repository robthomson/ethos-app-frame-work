...
local xdfly = assert(loadfile("app/modules/esc_tools/mfg/xdfly/init.lua"))()
local pages = assert(loadfile("app/modules/esc_tools/mfg/xdfly/pages.lua"))()
...
local function beginLoad(node, showLoader)
    ...
    if not api then
        node.state.error = "ESC_PARAMETERS_XDFLY unavailable."
    ...
    if api.setUUID then
        api.setUUID(utils.uuid("esc-xdfly-tool"))
    end
    ...
    api.setCompleteHandler(function()
        ...
        node.state.details = xdfly.details(parsed)
        node.state.activeFields = xdfly.activeFields(parsed)
    ...
    api.setErrorHandler(function(_, reason)
        ...
        node.state.error = tostring(reason or "ESC_PARAMETERS_XDFLY read failed.")
        escTools.clearLoader(node.app)
        node.app:_invalidateForm()
    end)
    ...
    if api.read() ~= true then
        unloadApi(node)
        node.state.loading = false
        node.state.error = "ESC_PARAMETERS_XDFLY read failed."
        escTools.clearLoader(node.app)
        return false
    end
...
function Page:open(ctx)
    ...
    local node = {
        app = ctx.app,
        baseTitle = ctx.item.title or "@i18n(app.modules.esc_tools.mfg.xdfly.name)@",
        title = ctx.item.title or "@i18n(app.modules.esc_tools.mfg.xdfly.name)@",
            ...
            items[#items + 1] = {
                ...
                press = function()
                    app:_enterItem(currentIndex, {
                        id = "esc-xdfly-" .. tostring(currentPage.id),
                        kind = "page",
