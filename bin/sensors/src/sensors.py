...
class SensorApp:
    ...
    def __init__(self, root, config_path=None):
        self.root = root
        self.root.title("RFSuite Simulator Sensor Tool")

        # The script layout is:
        #   GITREPO/bin/sensors/sensors.exe
        #   or GITREPO/bin/sensors/src/sensors.py
        # The source-side simulator sensor stubs live under:
        #   GITREPO/src/rfsuite/sim/sensors
        # Deploy mirrors them into:
    ...
    def _discover_simulator_targets(self):
        ...
        Returns a list of Path objects pointing to the deployed 'scripts'
        folder for each matching simulator target.
        """
        if getattr(sys, "frozen", False):
        ...
        for name, (var, mult, sensor_type, val_map) in self.controls.items():
            ...
            try:
                ...
                for dest in self.dest_paths:
                    # Write into the deployed runtime tree mirrored by deploy.py.
